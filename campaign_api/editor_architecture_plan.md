...
5. **RML forms generated per chain, comments as the human layer.** The in-game editor is a simple translation layer over recognizer output: each trigger chain renders as one RML form card. The comment block above the chain supplies the card's title/description (comments are CST nodes, so the recognizer hands them over for free); the chain's literal arguments become inputs, typed from the annotations (an `integer` param is a number field with no extra ceremony); `---@ui` comment hints optionally refine an input (`---@ui slider(1, 10)`, `---@ui choices(UnitDef)`); opaque `:Then` bodies display as read-only code. Edits write back through the CST node the input is bound to — the form never owns state, it's a view over the file. Two rules keep it honest: comments drive presentation ONLY (stripping them changes labels, never behavior), and `---@ui` is always an optional refinement over what the types already imply, so verbs never sprout mandatory ceremony. (Editor-side projections — decorations/inlays in VS Code anchored to the same nodes — remain possible later from the same recognizer output, but they are not this plan.)
